...
def write_masks(masks, labels, output_dir):
    other_area = torch.zeros(masks.shape[2], masks.shape[3], dtype=torch.bool)
    for i, mask in enumerate(masks):
        mask = mask[0].cpu()
        other_area = torch.bitwise_or(other_area, mask)
        cv2.imwrite(os.path.join(output_dir, labels[i][:labels[i].index('(')]+'.jpg'), mask.int().numpy() * 255)

    cv2.imwrite(os.path.join(output_dir, 'others.jpg'), (~other_area).int().numpy() * 255)
...
if __name__ == "__main__":
    ...
    for i, filename in enumerate(filenames):
        ...
        output_dir = os.path.join(output_root_dir, str(i))
        ...
        write_masks(masks, pred_phrases, output_dir)
