...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', default='/home/zhangwenyuan/enhancing-mono/GSDF/data/tnt/Barn/', type=str, help='input data path')
    opt = parser.parse_args()

    #### prepare data
    filenames = os.listdir(os.path.join(opt.data_dir,'images'))
    filenames = sorted(filenames)
    rgbs = []
    if 'Truck' in opt.data_dir:
        fovx=1.3986958653070056
        fovy=0.881621552836618
        W,H=979,546
    ...
    elif 'Barn' in opt.data_dir:
        fovx = 1.3963501908902032
        fovy = 0.8707081830477769
        W, H = 1954, 1090
        fx = W / (2 * math.tan(fovx / 2))
        fy = H / (2 * math.tan(fovy / 2))
        cx = (W - 1) / 2
    ...
    for rgb_filename in filenames:
        if not (rgb_filename.endswith('png') or rgb_filename.endswith('jpg')):
            continue
        ...
        rgb_origin = cv2.imread(os.path.join(opt.data_dir, 'images', rgb_filename))[:, :, ::-1]
    ...
    for i in tqdm(range(len(rgbs))):
        with torch.no_grad():
            ...
            # output
            img_idx = filenames[i][:6]     # {:06d}
            pred_depth = pred_depth.float().cpu().numpy()
            pred_normal = pred_normal.float().cpu().numpy()

            ###### todo: truck不需要，barn需要
            pred_depth = cv2.resize(pred_depth, (977, 545))
            pred_normal = cv2.resize(np.transpose(pred_normal,(1,2,0)), (977, 545))
            pred_normal = np.transpose(pred_normal,(2,0,1))
            ######

            np.save(os.path.join(opt.data_dir, 'priors', img_idx+'_depth.npy'), pred_depth)
            np.save(os.path.join(opt.data_dir, 'priors', img_idx+'_normal.npy'), pred_normal)
            cv2.imwrite(os.path.join(opt.data_dir, 'priors', img_idx+'_depth.png'), (pred_depth*(255/pred_depth.max())))
            cv2.imwrite(os.path.join(opt.data_dir, 'priors', img_idx+'_normal.png'), (np.transpose(pred_normal,(1,2,0))*255)[...,[2,1,0]])
