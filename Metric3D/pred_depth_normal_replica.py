# CUDA_VISIBLE_DEVICES=1 python pred_depth_normal_replica.py
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', default='/home/zhangwenyuan/enhancing-mono/monosdf-ours/data/replica/office4', type=str, help='input data path')
    ...
    camera_dict = np.load(os.path.join(opt.data_dir, 'cameras.npz'))
    scale_mats_np = [camera_dict['scale_mat_%d' % idx].astype(np.float32) for idx in range(3)]
    world_mats_np = [camera_dict['world_mat_%d' % idx].astype(np.float32) for idx in range(3)]
    ...
    for rgb_filename in filenames:
        if not rgb_filename.endswith('png'):
            continue
    ...
    for i in tqdm(range(len(rgbs))):
        with torch.no_grad():
            ...
            np.save(os.path.join(opt.data_dir, img_idx+'_depth.npy'), pred_depth.float().cpu().numpy())
            np.save(os.path.join(opt.data_dir, img_idx+'_normal.npy'), pred_normal.float().cpu().numpy())
            cv2.imwrite(os.path.join(opt.data_dir, img_idx+'_depth.png'), (pred_depth*(255/pred_depth.max())).int().cpu().numpy())
            cv2.imwrite(os.path.join(opt.data_dir, img_idx+'_normal.png'), (pred_normal.permute(1,2,0)*255).int().cpu().numpy()[...,[2,1,0]])
