...
class MonoSDFNetwork(nn.Module):
    ...
    def forward(self, input, indices, only_return_depth=False):
        # Parse model input
        ...
        warped_rgb_values = None
        warped_mask_values = None
        ...
        # __________________________________________________________________________________________________________
        # TODO: use batch instead of loop?
        if 'full_rgb_near' in input:
            warped_rgb_samps = []
            warped_mask_samps = []
            proj_valid_masks = []
            n_src = input["full_rgb_near"].shape[2]
            for inst_i in range(input["full_rgb_near"].shape[1]):
                if input["warp_valid"][0][inst_i]:
                    rgb_img_near = input["full_rgb_near"][:,inst_i].reshape(n_src,self.img_res[0], self.img_res[1], 3)   # [1,H,W,3]
                    img_res = rgb_img_near.shape[1:3]
                    mask_img_near = input["full_mask_near"][:,inst_i].reshape(n_src,self.img_res[0], self.img_res[1], 1)   # [1,H,W,1]
                    pose_near = input["pose_near"][0,inst_i]
                    use_warp_idx_mask = input["use_warp_idx_mask"][0,inst_i]
                    points_to_warped_flat = points_flat.reshape(num_pixels, N_samples, 3)[use_warp_idx_mask].reshape(-1,3)
                    # projected_uv, in_front = ours_util.project_points_to_another_view(points_to_warped_flat, intrinsics, pose_near)   # col, row
                    # projected_uv:[n_src,N,2], in_front:[n_src,N]
                    projected_uv, in_front = ours_util.project_points_to_another_view_neuralwarp(points_to_warped_flat, intrinsics.repeat([n_src, 1,1]), pose_near)
                    grid = ours_util.normalize(projected_uv.squeeze(0), img_res[0], img_res[1], clamp=10)
                    proj_valid_mask = (in_front.squeeze(0) & (grid < 1).all(dim=-1) & (grid > -1).all(dim=-1))

                    # [1,3,H,W] + [1,N_rays,N_samples,2] -> [1,3,N_rays,N_samples]. NOTE:grid[1]->col，grid[0]->row.
                    warped_rgb_samp = F.grid_sample(rgb_img_near.permute(0,3,1,2), grid.reshape(n_src,-1,N_samples,2), align_corners=True).permute(0,2,3,1)
                    warped_mask_samp = F.grid_sample(mask_img_near.permute(0,3,1,2), grid.reshape(n_src,-1,N_samples,2), align_corners=True).permute(0,2,3,1)
                    warped_mask_samp = warped_mask_samp>0

                    proj_valid_mask = proj_valid_mask.reshape(n_src, -1, N_samples)
                    warped_rgb_samp[~proj_valid_mask] = 0.
                    warped_rgb_samp *= warped_mask_samp

                    warped_rgb_samps.append(warped_rgb_samp)
                    warped_mask_samps.append(warped_mask_samp)
                    proj_valid_masks.append(proj_valid_mask)
                else:
                    warped_rgb_samps.append(None)
                    warped_mask_samps.append(None)
                    proj_valid_masks.append(None)
        # __________________________________________________________________________________________________________

        dirs = ray_dirs.unsqueeze(1).repeat(1,N_samples,1)
        dirs_flat = dirs.reshape(-1, 3)
        if only_return_depth:   # for aligning render depth with mono depth
            sdf = self.implicit_network.get_sdf_vals(points_flat)
        else:
        ...
        if only_return_depth:
            return {'depth_values': depth_values}
        ...
        # ___________________________________________ Pixel warping ________________________________________________
        warped_rgb_values_list = []
        warped_mask_values_list = []
        if 'full_rgb_near' in input:
            for inst_i in range(input["full_rgb_near"].shape[1]):
                if input["warp_valid"][0][inst_i]:
                    warped_rgb_values = torch.sum(weights[input["use_warp_idx_mask"][0,inst_i]][None,...,None] * warped_rgb_samps[inst_i], 2)
                    # (1) available projections in source view shouldn't be too few; (2) available projections in mask shouldn't be too few
                    warped_mask_values = (torch.sum(weights[input["use_warp_idx_mask"][0,inst_i]].unsqueeze(0) * proj_valid_masks[inst_i], 2, keepdim=True) > 0.01) & \
                                         (torch.sum(warped_mask_samps[inst_i], 2) > 0)   # if no points fall within mask
                    warped_rgb_values_list.append(warped_rgb_values)
                    warped_mask_values_list.append(warped_mask_values)
                else:
                    warped_rgb_values_list.append(None)
                    warped_mask_values_list.append(None)
        ...
        output = {
            ...
            'weights': weights,
            'rgb_values_warped': warped_rgb_values_list,      # [valid_inst_num, n_src, N_rays, 3]
            'mask_values_warped': warped_mask_values_list,    # [valid_inst_num, n_src, N_rays, 1]

        }

        if self.training:
            # ours eikonal loss: similar as neus
            in_sphere_mask = (points_flat<self.scene_bounding_sphere).all(-1) & (points_flat>-self.scene_bounding_sphere).all(-1)
            eikonal_points = points_flat[in_sphere_mask].detach()
