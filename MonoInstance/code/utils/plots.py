...
def plot(implicit_network, indices, plot_data, path, epoch, img_res, plot_nimgs, resolution, grid_boundary,  level=0):
    ...
    # TODO: use raw marching cubes for efficient. We do not observe much differences between raw mc and monosdf.
    surface_traces = get_surface_sliding_ours(path=path,
                                         epoch=epoch,
...
@torch.no_grad()
def get_surface_sliding_ours(path, epoch, sdf, resolution=100, grid_boundary=[-2.0, 2.0], return_mesh=False, level=0):
    resN = resolution
    level = level

    grid_min = [grid_boundary[0], grid_boundary[0], grid_boundary[0]]
    grid_max = [grid_boundary[1], grid_boundary[1], grid_boundary[1]]
    x_max, x_min = grid_max[0], grid_min[0]
    y_max, y_min = grid_max[1], grid_min[1]
    z_max, z_min = grid_max[2], grid_min[2]

    meshes = []
    x = np.linspace(grid_min[0], grid_max[0], resN)
    y = np.linspace(grid_min[1], grid_max[1], resN)
    z = np.linspace(grid_min[2], grid_max[2], resN)
    ...
    def evaluate(points):
        z = []
        for _, pnts in enumerate(torch.split(points, 100000, dim=0)):
            z.append(sdf(pnts).detach().cpu())
        z = torch.cat(z, axis=0)
        return z

    pts_sdf = evaluate(points.reshape(-1,3))
    z = pts_sdf.detach().cpu().numpy()

    vertices, triangles = mcubes.marching_cubes(z.reshape(resN,resN,resN), level)
    b_max_np = np.array(grid_max)
    b_min_np = np.array(grid_min)
    vertices = vertices / (resolution - 1.0) * (b_max_np - b_min_np)[None, :] + b_min_np[None, :]
    combined = trimesh.Trimesh(vertices, triangles)

    # if (not (np.min(z) > level or np.max(z) < level)):
    #     z = z.astype(np.float32)
    #     verts, faces, normals, values = measure.marching_cubes(
    #         volume=z.reshape(resN, resN, resN),  # .transpose([1, 0, 2]),
    #         level=level,
    #         spacing=(
    #             (x_max - x_min) / (resN - 1),
    #             (y_max - y_min) / (resN - 1),
