...
def calc_3d_metric(rec_meshfile, gt_meshfile, align=False):
    ...
    with open(os.path.join(os.path.dirname(rec_meshfile), 'result.txt'), 'w') as f:
        f.write(results)
    print('acc', accuracy_rec, 'comp', completion_rec,
          'prec', precision_ratio_rec, 'recall', completion_ratio_rec, 'f-score', fscore,
          'nacc', normal_acc, 'ncomp', normal_comp, 'navg', normal_avg)
