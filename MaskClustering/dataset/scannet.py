...
from utils.mask_backprojection import DEPTH_TRUNC
...
class ScanNetDataset:
    ...
    def __init__(self, seq_name) -> None:
        ...
        self.image_size = (640, 480)    # better than 648*484


    def get_frame_list(self, stride):
        image_list = os.listdir(self.rgb_dir)
        image_list = sorted(image_list, key=lambda x: int(x.split('.')[0][:6]))

        end = int(image_list[-1].split('.')[0][:6]) + 1
        frame_id_list = np.arange(0, end, stride)
        return list(frame_id_list)
    

    def get_intrinsics(self, frame_id):
        intrinsic_path = f'{self.intrinsic_dir}/intrinsic_depth.txt'        # keep consistent with 640*480
    ...
    def get_depth(self, frame_id):
        depth_path = os.path.join(self.depth_dir, '{:06d}_depth.png'.format(frame_id))
        depth = cv2.imread(depth_path, -1)
        depth = depth / self.depth_scale
        depth = np.where(depth > DEPTH_TRUNC, 0, depth)
        depth = depth.astype(np.float32)
        return depth


    def get_rgb(self, frame_id, change_color=True):
        rgb_path = os.path.join(self.rgb_dir, '{:06d}_rgb.png'.format(frame_id))
    ...
    def get_segmentation(self, frame_id, align_with_depth=False):
        segmentation_path = os.path.join(self.segmentation_dir, f'{frame_id:06d}_rgb.png')
