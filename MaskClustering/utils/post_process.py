...
def post_process(dataset, node_list, mask_point_clouds, scene_points, point_frame_matrix, frame_list, args):
    ...
    for node in (node_list):
        if len(node.mask_list) < 10: # objects merged from less than 2 masks are ignored
            continue
        
        pcld, point_ids = node.get_point_cloud(scene_points)
        pcld_list, point_ids_list = dbscan_process(pcld, point_ids, 0.5) # split the disconnected point cloud into different objects
    ...
    object_dict = export(dataset, total_point_ids_list, total_mask_list, args)
    return object_dict

def export_object_list(dataset, object_list, vis_dir):
    frame_list = dataset.get_frame_list(1)
    segment_imgs = [dataset.get_segmentation(i, align_with_depth=True) for i in frame_list]
    for i, object in enumerate(object_list):
        object_vis_dir = os.path.join(vis_dir, str(i))
        os.makedirs(object_vis_dir, exist_ok=True)
        segment_mask_imgs = {}
        for mask in object.mask_list:
            frame_id, mask_id = mask[0], mask[1]
            segment_img = segment_imgs[frame_list.index(frame_id)]
            if frame_id not in segment_mask_imgs:
                segment_mask_img = np.zeros_like(segment_img, dtype=np.uint8)[..., None].repeat(3, axis=-1)
            else:
                segment_mask_img = segment_mask_imgs[frame_id]
            segment_mask_img[segment_img == int(mask_id)] = 255
            segment_mask_imgs.update({frame_id: segment_mask_img})
        for frame_id, img in segment_mask_imgs.items():
            cv2.imwrite(os.path.join(object_vis_dir, str(frame_id) + '.jpg'), img)

def export_object_dict(dataset, vis_dir, object_dict:dict):
    frame_list = dataset.get_frame_list(1)
    segment_imgs = [dataset.get_segmentation(i, align_with_depth=True) for i in frame_list]
    instance_id = 0
    for i in object_dict.keys():
        object_vis_dir = os.path.join(vis_dir, str(instance_id))
        os.makedirs(object_vis_dir, exist_ok=True)
        segment_mask_imgs = {}
        if len(object_dict[i]['mask_list']) < 5:
            continue
        else:
            for mask in object_dict[i]['mask_list']:
                frame_id, mask_id = mask[0], mask[1]
                segment_img = segment_imgs[frame_list.index(frame_id)]
                if frame_id not in segment_mask_imgs:
                    segment_mask_img = np.zeros_like(segment_img, dtype=np.uint8)[..., None].repeat(3, axis=-1)
